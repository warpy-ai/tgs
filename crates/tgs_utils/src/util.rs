...
pub fn get_terminal() -> std::io::Stdin {
    std::io::stdin()
}
...
pub fn initialize_job_control() -> anyhow::Result<()> {
    ...
    loop {
        ...
        if unistd::tcgetpgrp(shell_terminal.as_raw_fd())? == shell_pgid {
            break;
        } else {
    ...
    let shell_terminal = get_terminal();
    let temp_result = unistd::tcsetpgrp(shell_terminal.as_raw_fd(), shell_pgid);
