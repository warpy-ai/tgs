...
use std::io::Stdin;
use std::{fmt, os::fd::AsRawFd, process::ExitStatus};
use thiserror::Error;
...
impl TerminalState {
    fn new(new_pgid: Pid) -> TerminalState {
        ...
        let shell_terminal_fd = shell_terminal.as_raw_fd();
        unistd::tcsetpgrp(shell_terminal_fd, new_pgid).unwrap();
...
impl Drop for TerminalState {
    fn drop(&mut self) {
        ...
        let shell_terminal = util::get_terminal();
        let shell_terminal_fd = shell_terminal.as_raw_fd();
        unistd::tcsetpgrp(shell_terminal_fd, self.prev_pgid).unwrap();
